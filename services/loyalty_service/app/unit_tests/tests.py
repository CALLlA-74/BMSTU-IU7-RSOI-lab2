...
def init_db(db: Session, init_data: list):
    for data in init_data:
        loyalty = LoyaltyService.create_loyalty(data['username'], db)
...
async def test_get_by_username_success():
    try:
        for loyalty in loyalties:
            received_loyalty = LoyaltyService.get_loyalty(loyalty['username'], test_db).get_dto_model()
        ...
        received_loyalty = LoyaltyService.get_loyalty('username', test_db).get_dto_model()
        loyalty = {
            'username:': 'username',
            'reservationCount': 0,
            'status': 'BRONZE',
...
async def test_update_by_username_success():
    try:
        for i in range(25):
            ...
            loyalty = LoyaltyService.update_loyalty(update_data, loyalties[0]['username'], test_db).get_dto_model()
            correct_loyalty = {
                'reservationCount': i,
...
async def test_update_by_username_not_found():
    try:
        ...
        LoyaltyService.update_loyalty(update_data, 'unknown_name', test_db).get_dto_model()
